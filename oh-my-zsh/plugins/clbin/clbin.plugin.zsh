...
# Original found at https://www.shellperson.net/clbin-pastebin-script/
...
DESCRIPTION
  Upload data and fetch URL from the pastebin https://clbin.com
	...
	if [ -t 0 ]; then
	  ...
	  if [ "$*" ]; then
		...
		if [ -f "$*" ]; then
		  ...
		  echo "$*"
		fi | curl -F 'clbin=<-' https://clbin.com
	  else
